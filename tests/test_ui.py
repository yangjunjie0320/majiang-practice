...
def test_ting_flow(app_page):
    ...
    assert page.locator("#problem .tile").count() >= 1  # 副露最多 4 组，暗牌 1-13 张
...
def test_discard_flow(app_page):
    ...
    # 副露 0-4 组时暗牌 14/11/8/5/2 张
    assert page.locator("#rows > div").nth(0).locator("button.tile").count() in (2, 5, 8, 11, 14)
